...
void UART1(void *pvNull) {
  //Variables
  TickType_t xLastWakeTime = xTaskGetTickCount();
  UPAD_UART uart1(UART_NUM_1, ESP_INTR_FLAG_LEVEL2, uPad_pin_gpio33, uPad_pin_gpio34);
  ...
  for (;;) {
    //Write
    while ((count < 5) && (uart1.GET() == uPad_DONE)) {
      if (from_user.data[0] == CMD_Starting) {
        uart1.EXP(&from_user.data[++count]);
      } else {
        uart1.EXP(&from_user.data[0]);
      }
    }
    if (count == 5) {
...
void RGBLED(void *pvNull) {
  ...
  *hex2hsv.config = uPad_col_HSV;
  ...
  for (;;) {
    xQueuePeek(CmdMail, &cmd, 0);
    if (counter == 0) {
      for (int lednum = 0; lednum < 2; lednum++) {
        for (int i = 0; i < 24; i++) {
          rgbled.IMP(&code_low);
        }
